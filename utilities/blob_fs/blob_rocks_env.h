...
// The user space file system impl of rocksdb Env.
// 1. must specify DBOptions's db_log_dir param
class BlobRocksEnv : public rocksdb::EnvWrapper {
public:
  explicit BlobRocksEnv(BlobFS* fs);
  ...
  // Create directory if missing. Return Ok if it exists, or successful in creating.
